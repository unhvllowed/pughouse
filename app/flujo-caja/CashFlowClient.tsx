...
interface CashFlowEntry {
  ...
  date: Date; description: string | null; reference: string | null;
}
...
export default function CashFlowClient({
  ...
  initialDailyDelta: DailyDelta[];
  initialMonthlyComparison: MonthlyData[];
  ...
  const dailyDelta = initialDailyDelta;
  const monthlyComparison = initialMonthlyComparison;
  const topProducts = initialTopProducts;
  
  const [selectedMonth, setSelectedMonth] = useState(initialMonth);
  const [deltaEnd, setDeltaEnd] = useState(initialTo);

  // Excel export modal
  const [showExcel, setShowExcel] = useState(false);
  const [excelMonth, setExcelMonth] = useState(initialMonth);
  ...
  const handleMonthChange = (val: string) => {
    ...
    router.push(`/flujo-caja?month=${val}&to=${deltaEnd}`, { scroll: false });
  };
  
  const handleDeltaEndChange = (val: string) => {
    setDeltaEnd(val);
    router.push(`/flujo-caja?month=${selectedMonth}&to=${val}`, { scroll: false });
  };
  ...
  function downloadExcel() {
    ...
    const filtered = entries.filter((e) => {
      const eDate = typeof e.date === 'string' ? e.date : new Date(e.date).toISOString();
      return eDate.startsWith(excelMonth);
    });
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20, flexWrap: "wrap", gap: 12 }}>
            <div>
              ...
              <span style={{ fontSize: 12, color: "var(--text-muted)" }}>
                {new Date(initialFrom + "T12:00:00").toLocaleDateString("es-CL", { day: "numeric", month: "long" })}
                {" → "}
                {new Date(initialTo + "T12:00:00").toLocaleDateString("es-CL", { day: "numeric", month: "long", year: "numeric" })}
              </span>
            </div>
            <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
              ...
              <input
                ...
                onChange={(e) => handleDeltaEndChange(e.target.value)}
                style={{ width: 150, padding: "4px 8px", fontSize: 12 }}
              />
              <button
                className="btn btn-secondary btn-sm"
                style={{ padding: "4px 10px", fontSize: 11 }}
                onClick={() => handleDeltaEndChange(toLocalDate(new Date()))}
                title="Volver a hoy"
              >Hoy</button>
            </div>
          </div>
